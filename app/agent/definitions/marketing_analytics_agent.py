"""
NapsterTec AI - Marketing Analytics Intelligence Agent
Module: app/agent/definitions/marketing_analytics_agent.py
"""
import logging
...
class MarketingAnalyticsIntelligenceAgent(BaseAgent):
    def __init__(self, **kwargs):
        metadata = AgentMetadata(
            name="marketing_analytics",
            display_name="Marketing Analytics Intelligence",
    ...
    async def execute(self, context: AgentContext) -> AgentResult:
        ...
        try:
            ...
            b_res = await self.invoke_tool("marketing_analytics_context_builder", {"query": search_query}, context)
            ...
            e_res = await self.invoke_tool("marketing_analytics_evaluator", {"context": iso_context}, context)
            ...
            s_res = await self.invoke_tool("marketing_analytics_artifact_saver", {"artifact": artifact}, context)
            ...
            impact = artifact.get('business_impact', {})
            platforms = artifact.get('platform_performance', [])
            audiences = artifact.get('audience_performance', [])
            top_platform = platforms[0].get('platform') if platforms else "N/A"
            top_audience = audiences[0].get('segment') if audiences else "N/A"

            summary = (
                f"**Marketing Analytics Execution Report**\n\n"
                f"Campaign Performance: {artifact.get('campaign_performance')}\n"
                f"Platform Ranking: #1 {top_platform}\n"
                f"Audience Ranking: #1 {top_audience}\n"
                f"Lead Generation: {impact.get('qualified_leads')} Qualified Leads\n"
                f"Business Impact: {impact.get('meetings_scheduled')} Meetings | {impact.get('revenue_pipeline_generated')} Pipeline\n"
                f"Optimization Recommendations: {len(artifact.get('optimization_recommendations', []))} Insights Generated\n"
                f"ROI Summary: {impact.get('marketing_roi')}\n\n"
                f"**Artifact Generation:**\n"
                f"Artifact Created: MarketingAnalyticsArtifact\n"
            ...
            logger.error(f"[Marketing Analytics Agent] Execution failed: {str(e)}", exc_info=True)
